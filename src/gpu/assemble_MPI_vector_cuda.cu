...
extern EXTERN_LANG
void FC_FUNC_(get_boundary_accel_device_ptr_by_interface,
              GET_BOUNDARY_ACCEL_DEVICE_PTR_BY_INTERFACE)(long* Mesh_pointer,
                                                          const int* iinterface,
                                                          const int* FORWARD_OR_ADJOINT,
                                                          long* send_accel_device_ptr) {

  TRACE("get_boundary_accel_device_ptr_by_interface");

  Mesh* mp = (Mesh*)(*Mesh_pointer);
  *send_accel_device_ptr = 0;

  if (mp->size_mpi_buffer > 0) {
    const int iface = *iinterface - 1;
    ...
    if (d_send_buffer != NULL) {
      *send_accel_device_ptr = (long)(d_send_buffer + 3 * iface * mp->max_nibool_interfaces_ext_mesh);
    }
  }
}
