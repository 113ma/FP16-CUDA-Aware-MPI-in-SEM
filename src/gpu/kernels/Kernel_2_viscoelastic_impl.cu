...
__device__ __forceinline__ void sum_hprime_xi(int I, int J, int K,
                                               __nv_bfloat16* tempxl, __nv_bfloat16* tempyl, __nv_bfloat16* tempzl,
                                               realw* sh_tempx, realw* sh_tempy, realw* sh_tempz, 
                                               realw* sh_hprime) {
    // 1. 预计算基地址（内存访问优化核心）
    const int base_idx = K * NGLL2 + J * NGLLX;
    
    // 2. 使用寄存器变量优化累加过程
    __nv_bfloat16 reg_sumx = __float2bfloat16(0.0f);
    __nv_bfloat16 reg_sumy = __float2bfloat16(0.0f);
    __nv_bfloat16 reg_sumz = __float2bfloat16(0.0f);
    
    // 3. 预加载hprime值到寄存器（减少共享内存访问）
    #pragma unroll
    for (int l = 0; l < NGLLX; l++) {
        // 预加载到寄存器数组
        realw preload_hprime = sh_hprime[l*NGLLX+I];
        __nv_bfloat16 h_fac = __float2bfloat16(preload_hprime);
        
        // 4. 使用预计算索引
        const int idx = base_idx + l;
        
        // 5. 使用预转换的缩放因子（减少类型转换开销）
        __nv_bfloat16 h_valx = __float2bfloat16(sh_tempx[idx]);
        __nv_bfloat16 h_valy = __float2bfloat16(sh_tempy[idx]);
        __nv_bfloat16 h_valz = __float2bfloat16(sh_tempz[idx]);
        
        // 6. FMA累加到寄存器变量
        reg_sumx = __hfma(h_valx, h_fac, reg_sumx);
        reg_sumy = __hfma(h_valy, h_fac, reg_sumy);
        reg_sumz = __hfma(h_valz, h_fac, reg_sumz);
    }
...
__device__ __forceinline__ void sum_hprime_eta(int I, int J, int K,
    ...
    for (int l = 0; l < NGLLX; l++) {
        // 预加载到寄存器数组
        realw preload_hprime = sh_hprime[l*NGLLX+J];
        __nv_bfloat16 h_fac = __float2bfloat16(preload_hprime);
        
        // 4. 使用预计算索引
        const int idx = base_idx + l * NGLLX;
...
__device__ __forceinline__ void sum_hprime_gamma(int I, int J, int K,
    ...
    for (int l = 0; l < NGLLX; l++) {
        // 预加载到寄存器数组
        realw preload_hprime = sh_hprime[l*NGLLX+K];
        __nv_bfloat16 h_fac = __float2bfloat16(preload_hprime);
        
        // 4. 使用预计算索引
        const int idx = l * NGLL2 + base_idx;
...
__device__ __forceinline__ void sum_hprimewgll_xi(int I, int J, int K,
                                                 ...
                                                 realw* sh_tempx, realw* sh_tempy, realw* sh_tempz, 
                                                 realw* sh_hprimewgll) {
    __nv_bfloat16 sumx = __float2bfloat16(0.0f);
    __nv_bfloat16 sumy = __float2bfloat16(0.0f);
    __nv_bfloat16 sumz = __float2bfloat16(0.0f);

    #pragma unroll
    for (int l = 0; l < NGLLX; l++) {
        __nv_bfloat16 h_valx = __float2bfloat16(sh_tempx[K*NGLL2+J*NGLLX+l] );
        __nv_bfloat16 h_valy = __float2bfloat16(sh_tempy[K*NGLL2+J*NGLLX+l] );
        __nv_bfloat16 h_valz = __float2bfloat16(sh_tempz[K*NGLL2+J*NGLLX+l] );
        __nv_bfloat16 h_fac = __float2bfloat16(sh_hprimewgll[I*NGLLX+l] );

        sumx = __hfma(h_valx, h_fac, sumx);
        sumy = __hfma(h_valy, h_fac, sumy);
        sumz = __hfma(h_valz, h_fac, sumz);
    }
...
__device__ __forceinline__ void sum_hprimewgll_eta(int I, int J, int K,
    ...
    #pragma unroll
    for (int l = 0; l < NGLLX; l++) {
        __nv_bfloat16 h_valx = __float2bfloat16(sh_tempx[K*NGLL2+l*NGLLX+I] );
        __nv_bfloat16 h_valy = __float2bfloat16(sh_tempy[K*NGLL2+l*NGLLX+I]);
        __nv_bfloat16 h_valz = __float2bfloat16(sh_tempz[K*NGLL2+l*NGLLX+I] );
        __nv_bfloat16 h_fac = __float2bfloat16(sh_hprimewgll[J*NGLLX+l] );
...
__device__ __forceinline__ void sum_hprimewgll_gamma(int I, int J, int K,
    ...
    #pragma unroll
    for (int l = 0; l < NGLLX; l++) {
        __nv_bfloat16 h_valx = __float2bfloat16(sh_tempx[l*NGLL2+J*NGLLX+I]  );
        __nv_bfloat16 h_valy = __float2bfloat16(sh_tempy[l*NGLL2+J*NGLLX+I] );
        __nv_bfloat16 h_valz = __float2bfloat16(sh_tempz[l*NGLL2+J*NGLLX+I] );
        __nv_bfloat16 h_fac = __float2bfloat16(sh_hprimewgll[K*NGLLX+l] ); // 修复了缺少的右括号
...
__device__ __forceinline__ void get_spatial_derivatives(realw* xixl,realw* xiyl,realw* xizl,
                                                         ...
                                                         __nv_bfloat16* tempx1l, __nv_bfloat16* tempy1l, __nv_bfloat16* tempz1l,  // 改为bfloat16
                                                         __nv_bfloat16* tempx2l, __nv_bfloat16* tempy2l, __nv_bfloat16* tempz2l,  // 改为bfloat16
                                                         __nv_bfloat16* tempx3l, __nv_bfloat16* tempy3l, __nv_bfloat16* tempz3l,  // 改为bfloat16
                                                         realw* sh_tempx,realw* sh_tempy,realw* sh_tempz,
                                                         realw* sh_hprime_xx,
                                                         __nv_bfloat16* duxdxl, __nv_bfloat16* duxdyl, __nv_bfloat16* duxdzl,     // 改为bfloat16
                                                         __nv_bfloat16* duydxl, __nv_bfloat16* duydyl, __nv_bfloat16* duydzl,     // 改为bfloat16
                                                         __nv_bfloat16* duzdxl, __nv_bfloat16* duzdyl, __nv_bfloat16* duzdzl,     // 改为bfloat16
  ...
  __nv_bfloat16 h_tempx1, h_tempy1, h_tempz1;
  __nv_bfloat16 h_tempx2, h_tempy2, h_tempz2;
  __nv_bfloat16 h_tempx3, h_tempy3, h_tempz3;
  if (ispec_irreg >= 0 && ipass==0 ){
    ...
    *jacobianl = 1.f / ((*xixl)*((*etayl)*(*gammazl)-(*etazl)*(*gammayl))
    ...
    sum_hprime_xi(I, J, K, &h_tempx1, &h_tempy1, &h_tempz1, 
                  sh_tempx, sh_tempy, sh_tempz, sh_hprime_xx);
    sum_hprime_eta(I, J, K, &h_tempx2, &h_tempy2, &h_tempz2, 
                   sh_tempx, sh_tempy, sh_tempz, sh_hprime_xx);
    sum_hprime_gamma(I, J, K, &h_tempx3, &h_tempy3, &h_tempz3, 
                     sh_tempx, sh_tempy, sh_tempz, sh_hprime_xx);
    *tempx1l = h_tempx1;
    *tempy1l = h_tempy1;
    *tempz1l = h_tempz1;
  ...
  if (ispec_irreg >= 0 ){
    // irregular_element
      __nv_bfloat16 h_xix = __float2bfloat16(*xixl);
        __nv_bfloat16 h_xiy = __float2bfloat16(*xiyl);
        __nv_bfloat16 h_xiz = __float2bfloat16(*xizl);
        __nv_bfloat16 h_etax = __float2bfloat16(*etaxl);
        __nv_bfloat16 h_etay = __float2bfloat16(*etayl);
        __nv_bfloat16 h_etaz = __float2bfloat16(*etazl);
        __nv_bfloat16 h_gammax = __float2bfloat16(*gammaxl);
        __nv_bfloat16 h_gammay = __float2bfloat16(*gammayl);
        __nv_bfloat16 h_gammaz = __float2bfloat16(*gammazl);

        *duxdxl = __hfma(h_xix, *tempx1l, __hfma(h_etax, *tempx2l, __hmul(h_gammax, *tempx3l)));
        *duxdyl = __hfma(h_xiy, *tempx1l, __hfma(h_etay, *tempx2l, __hmul(h_gammay, *tempx3l)));
        *duxdzl = __hfma(h_xiz, *tempx1l, __hfma(h_etaz, *tempx2l, __hmul(h_gammaz, *tempx3l)));

        *duydxl = __hfma(h_xix, *tempy1l, __hfma(h_etax, *tempy2l, __hmul(h_gammax, *tempy3l)));
        *duydyl = __hfma(h_xiy, *tempy1l, __hfma(h_etay, *tempy2l, __hmul(h_gammay, *tempy3l)));
        *duydzl = __hfma(h_xiz, *tempy1l, __hfma(h_etaz, *tempy2l, __hmul(h_gammaz, *tempy3l)));

        *duzdxl = __hfma(h_xix, *tempz1l, __hfma(h_etax, *tempz2l, __hmul(h_gammax, *tempz3l)));
        *duzdyl = __hfma(h_xiy, *tempz1l, __hfma(h_etay, *tempz2l, __hmul(h_gammay, *tempz3l)));
        *duzdzl = __hfma(h_xiz, *tempz1l, __hfma(h_etaz, *tempz2l, __hmul(h_gammaz, *tempz3l)));
    } 
  else{
    // regular element
        __nv_bfloat16 h_xix_regular = __float2bfloat16(xix_regular);
        *duxdxl = __hmul(h_xix_regular, *tempx1l);
        *duxdyl = __hmul(h_xix_regular, *tempx2l);
        *duxdzl = __hmul(h_xix_regular, *tempx3l);
        *duydxl = __hmul(h_xix_regular, *tempy1l);
        *duydyl = __hmul(h_xix_regular, *tempy2l);
        *duydzl = __hmul(h_xix_regular, *tempy3l);
        *duzdxl = __hmul(h_xix_regular, *tempz1l);
        *duzdyl = __hmul(h_xix_regular, *tempz2l);
        *duzdzl = __hmul(h_xix_regular, *tempz3l);
...
//            COMPUTE_AND_STORE_STRAIN = .true. (always true for attenuation)
  ...
  realw fac1,fac2,fac3;
       ...
       __nv_bfloat16 h_sigma_xx, h_sigma_yy, h_sigma_zz;  // bfloat16应力
    __nv_bfloat16 h_sigma_xy, h_sigma_xz, h_sigma_yz;  // bfloat16应力
    __nv_bfloat16 duxdxl_plus_duydyl, duxdxl_plus_duzdzl;
    __nv_bfloat16 duydyl_plus_duzdzl, duxdyl_plus_duydxl;
    __nv_bfloat16 duzdxl_plus_duxdzl, duzdyl_plus_duydzl;
    __nv_bfloat16 kappal, mul, lambdalplus2mul, lambdal;
    __nv_bfloat16 duxdxl, duxdyl, duxdzl;
    __nv_bfloat16 duydxl, duydyl, duydzl;
    __nv_bfloat16 duzdxl, duzdyl, duzdzl;
    __nv_bfloat16 tempx1l, tempy1l, tempz1l;
    __nv_bfloat16 tempx2l, tempy2l, tempz2l;
    __nv_bfloat16 tempx3l, tempy3l, tempz3l;
    __nv_bfloat16 h_epsilondev_xx_loc, h_epsilondev_yy_loc;
    __nv_bfloat16 h_epsilondev_xy_loc, h_epsilondev_xz_loc, h_epsilondev_yz_loc;
    __nv_bfloat16 h_epsilondev_trace_loc, h_templ;
  realw f_tempx1l,f_tempx2l,f_tempx3l,f_tempy1l,f_tempy2l,f_tempy3l,f_tempz1l,f_tempz2l,f_tempz3l;
 ...
 h_epsilondev_trace_loc = __hadd(__hadd(duxdxl, duydyl), duzdzl);
h_templ = __hmul(__float2bfloat16(1.0f/3.0f), h_epsilondev_trace_loc);

// 修正epsilon计算
h_epsilondev_xx_loc = __hsub(duxdxl, h_templ);
h_epsilondev_yy_loc = __hsub(duydyl, h_templ);
h_epsilondev_xy_loc = __hmul(__float2bfloat16(0.5f), __hadd(duxdyl, duydxl));
h_epsilondev_xz_loc = __hmul(__float2bfloat16(0.5f), __hadd(duzdxl, duxdzl));
h_epsilondev_yz_loc = __hmul(__float2bfloat16(0.5f), __hadd(duzdyl, duydzl));

epsilondev_xx_loc = __bfloat162float(h_epsilondev_xx_loc);
epsilondev_yy_loc = __bfloat162float(h_epsilondev_yy_loc);
epsilondev_xy_loc = __bfloat162float(h_epsilondev_xy_loc);
epsilondev_xz_loc = __bfloat162float(h_epsilondev_xz_loc);
epsilondev_yz_loc = __bfloat162float(h_epsilondev_yz_loc);
epsilondev_trace_loc = __bfloat162float(h_epsilondev_trace_loc);
templ = __bfloat162float(h_templ);
  ...
  duxdxl_plus_duydyl = __hadd(duxdxl, duydyl);
    duxdxl_plus_duzdzl = __hadd(duxdxl, duzdzl);
    duydyl_plus_duzdzl = __hadd(duydyl, duzdzl);
    duxdyl_plus_duydxl = __hadd(duxdyl, duydxl);
    duzdxl_plus_duxdzl = __hadd(duzdxl, duxdzl);
    duzdyl_plus_duydzl = __hadd(duzdyl, duydzl);

  realw kappa_full = get_global_cr(&d_kappav[offset]);
    realw mu_full = get_global_cr(&d_muv[offset]);
 

#define FOUR_THIRDS (4.0f/3.0f)  // 移出函数定义

 kappal = __float2bfloat16(kappa_full);  // 转换为bfloat16
mul = __float2bfloat16(mu_full);        // 转换为bfloat16

// full anisotropic case, stress calculations
if (ANISOTROPY){
    // 各向异性情况，根据实际需要修改
} else {
    lambdalplus2mul = __hadd(kappal, __hmul(__float2bfloat16(FOUR_THIRDS), mul));
    lambdal = __hsub(lambdalplus2mul, __hmul(__float2bfloat16(2.0f), mul));

    h_sigma_xx = __hfma(lambdalplus2mul, duxdxl, 
                       __hmul(lambdal, duydyl_plus_duzdzl));
    h_sigma_yy = __hfma(lambdalplus2mul, duydyl, 
                       __hmul(lambdal, duxdxl_plus_duzdzl));
    h_sigma_zz = __hfma(lambdalplus2mul, duzdzl, 
                       __hmul(lambdal, duxdxl_plus_duydyl));
    
    h_sigma_xy = __hmul(mul, duxdyl_plus_duydxl);
    h_sigma_xz = __hmul(mul, duzdxl_plus_duxdzl);
    h_sigma_yz = __hmul(mul, duzdyl_plus_duydzl);

    sigma_xx = __bfloat162float(h_sigma_xx);
    sigma_yy = __bfloat162float(h_sigma_yy);
    sigma_zz = __bfloat162float(h_sigma_zz);
    sigma_xy = __bfloat162float(h_sigma_xy);
    sigma_xz = __bfloat162float(h_sigma_xz);
    sigma_yz = __bfloat162float(h_sigma_yz);
  }
 ...
 f_tempx1l = __bfloat162float(tempx1l);
f_tempx2l = __bfloat162float(tempx2l);
f_tempx3l = __bfloat162float(tempx3l);

f_tempy1l = __bfloat162float(tempy1l);
f_tempy2l = __bfloat162float(tempy2l);
f_tempy3l = __bfloat162float(tempy3l);

f_tempz1l = __bfloat162float(tempz1l);
f_tempz2l = __bfloat162float(tempz2l);
f_tempz3l = __bfloat162float(tempz3l);
  // adds gravity term
  if (gravity){
    sum_terms1 += rho_s_H1;
    sum_terms2 += rho_s_H2;
